...
public class AtwaterServer {
	public static void main(String [] args) throws Exception {
		MovieTicketImplementation atwMovieBookingImplementation=new MovieTicketImplementation("ATW");
		Endpoint endpoint= Endpoint.publish("http://localhost:7000/atwater",atwMovieBookingImplementation);
		System.out.println("Atwater Web Service is published Successfully "+endpoint.isPublished());
		while(true) {
			requestHandler(atwMovieBookingImplementation);
		}
	}
	
	public static void requestHandler(MovieTicketImplementation atwMovieBookingImplementation) throws IOException{

		//Send request to get data.
		try {
			DatagramSocket socket = new DatagramSocket(3800);
